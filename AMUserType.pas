...
uses
  ...
  Vcl.ExtCtrls,mmsystem,System.Generics.Collections,TypInfo,Vcl.ComCtrls,Types;
 type
...
function amStr(value:Tcolor):string; overload;  inline;
...
{поиск в байтах}
function PosBytes_IncCallback (BPatern:Tbytes;BSource:Tbytes;CountBPatern,CountBSource:int64;var Callback:int64):int64;
function PosBytes_Inc (BPatern:Tbytes;BSource:Tbytes;CountBPatern,CountBSource:int64):int64;
function PosBytes_Careta (BPatern,BSource:Tbytes;CountBPatern,CountBSource,offset:int64;var Callback:int64):int64;

  type
         TAmSetSymbol  = set of byte;
  type   PPosBytes_KeyValue=^TPosBytes_KeyValue;
         TPosBytes_KeyValue =record
          ...
          SymbolEnd :TAmSetSymbol//символы byte которые означают что это конец записи SymbolEnd:=[13];
...
Procedure PosBytes_KeyValue(P:TPosBytes_KeyValue;IsBeginFile:boolean;var ReCallback,ReEtap,ReEnd,ReLast,ReFerst:int64);
Procedure PosBytes_SizeFixed(P:TPosBytes_KeyValue;IsBeginFile:boolean;var ReCallback,ReEtap,ReEnd,ReLast,ReFerst:int64);
...
type TamPostMessageResult<T1,T2> = class (TamPostMessage<T1>)
     ...
     function PostMessageLock(aHandle:HWND;aMessage:UINT;SecondWaitFor:Integer):TPostMessageCS;
...
type TamResource = class
      ...
      public
        ...
        function          BeginUpdate(ResourceNameIndificator:string):Cardinal;
...
Type
 AmFile = record
   ...
   Class procedure ReCteateFileData  ( aFile : String;MinSizeFile:int64;LastData:TDatetime;AIncHour:integer=10);inline; static;
...
implementation
uses AmLogTo,AmHlpType,AmList,Wininet,JclDebug,JclSysUtils;
...
{ TTypeKind = (tkUnknown, tkInteger, tkChar, tkEnumeration, tkFloat,
  ...
  tkClassRef, tkPointer, tkProcedure, tkMRecord);}
     ...
     case V.Kind of
        ...
        tkPointer       : F.Value:=      '$'+ IntToHex(Integer(V.AsType<Pointer>));
...
begin
  ...
  begin
     ...
     OutTrack.CodeProc:=          Pointer(Cardinal(J.Address) - J.OffsetFromProcName);
...
var f:string;
 function LocFun(uper:boolean;Input,ValueTrue:string;isTotch:boolean):string;
 var S:string;
 begin
    Result:=Input;
    if uper then
 ...
 begin
     ...
     if y then
     begin
        if yIs2 then Result:=LocFun(m,Result,'yy',true)
        else         Result:=LocFun(m,Result,'yyyy',true);
     end;
 end;
begin
    f:='';
    f:=LocFun(d,f,'dd',true);
    f:=LocFun(m,f,'mm',true);
    ...
    begin
          if f<>'' then
          f:= f+'" "';
          f:=LocFun(h,f,'hh',false);
          f:=LocFun(n,f,'nn',true);
          f:=LocFun(s,f,'ss',true);
          f:=LocFun(z,f,'zzz',true);
    end;
  ...
  try
     try
          ...
          begin
             ...
             Error:='Тип чтения ['+ Ftype.ToString +'] и записи [' +string(InfoVariable.Name)+ '] не совпадают для поля['+PathFieldName+']  в типе данных['+Node.RootTypeInfo.Name+']'
             else
             Error:='Тип чтения ['+ 'неопределено' +'] и записи [' +InfoVariable.Name+ '] не совпадают для поля['+PathFieldName+']  в типе данных['+Node.RootTypeInfo.Name+']';
             Result:=false;
          end;
      ...
      on e:exception do
           begin
             if Assigned(Ftype) then
             Error:='Error конвертации типа данных '+e.Message+' Тип чтения ['+ Ftype.ToString +'] и записи [' +InfoVariable.Name+ '] не совпадают для поля['+PathFieldName+']  в типе данных['+Node.RootTypeInfo.Name+']'
             else
             Error:='Error конвертации типа данных '+e.Message+'  Тип чтения ['+ 'неопределено' +'] и записи [' +InfoVariable.Name+ '] не совпадают для поля['+PathFieldName+']  в типе данных['+Node.RootTypeInfo.Name+']';
             Result:=false;
           end;
     ...
     //  s:= TObject(Field.Parent.Handle.TypeData).ClassName;
       ...
       begin
        ...
        begin
           ...
           Item.Instance:=  Pointer(Cardinal(RootInstance) + Field.Offset);
    ...
    tkClassRef, tkPointer, tkProcedure, tkMRecord);   }
         ...
         // Showmessage(GetEnumname(TypeInfo(TTypeKind),Ord(Ftype.TypeKind)));
          ...
          begin
             ...
             begin
               Result:=false;
               Error:=' Такой тип данных не поддерживается для конвертации в Double Тип чтения ['+ Ftype.ToString +'] для поля['+PathFieldName+']  в типе данных['+Node.RootTypeInfo.Name+']';
          ...
          begin
             Error:='Тип чтения ['+ 'неопределено' +'] не совпадают для поля['+PathFieldName+']  в типе данных['+Node.RootTypeInfo.Name+']';
             Result:=false;
          end;
      ...
      on e:exception do
           begin
             if Assigned(Ftype) then
             Error:='Error конвертации типа данных '+e.Message+' Тип чтения ['+ Ftype.ToString +'] для поля['+PathFieldName+']  в типе данных['+Node.RootTypeInfo.Name+']'
             else
             Error:='Error конвертации типа данных '+e.Message+'  Тип чтения ['+ 'неопределено' +'] для поля['+PathFieldName+']  в типе данных['+Node.RootTypeInfo.Name+']';
             Result:=false;
           end;
    ...
    tkClassRef, tkPointer, tkProcedure, tkMRecord);   }
         ...
         // Showmessage(GetEnumname(TypeInfo(TTypeKind),Ord(Ftype.TypeKind)));
          ...
          begin
             ...
             begin
               Result:=false;
               Error:=' Такой тип данных не поддерживается для конвертации в строку Тип чтения ['+ Ftype.ToString +'] для поля['+PathFieldName+']  в типе данных['+Node.RootTypeInfo.Name+']';
...
begin
      ...
      case RootTypeInfo.Kind of
           ...
           begin
             RootType:=nil;
             RootInstance:=nil;
             Error:='Тип данных ['+RootTypeInfo.Name+'] не является tkClass,tkRecord он равен['+GetEnumname( TypeInfo(TTypeKind), Ord(RootTypeInfo.Kind) )+']';
             Result:=false;
           end;
             ...
             begin
               ...
               // else
                ...
                begin
                    ...
                    begin
                      FMember[i].Member:=mem;
                      FMember[i].prtIndexPrm:=prtIndexPrm;
...
Class procedure AmFile.ReCteateFileData  ( aFile : String;MinSizeFile:int64;LastData:TDatetime;AIncHour:integer=10);
begin
 ...
 //  AParent.Text;
    case V.Typ of
       { jdtString:      FL.Items.AddChildFirst(AParent,'String:'+V.Value);
        ...
        jdtFloat:       AParent.Text :=   Loc_GetValue('Float',AmStr(V.FloatValue));
        jdtDateTime:    AParent.Text :=   Loc_GetValue('DateTime',AmStr(V.DateTimeValue));
...
function  TamResource.BeginUpdate(ResourceNameIndificator:string):Cardinal;
begin
  //sult:=BeginUpdateResource(Pchar(Application.ExeName),true);
 // showmessage( BeginUpdate(Application.ExeName).ToString );
 REsult:=0;
end;
...
function TamPostMessageResult<T1,T2>.PostMessageLock(aHandle:HWND;aMessage:UINT;SecondWaitFor:Integer):TPostMessageCS;
var R:bool;
    dwResult:Cardinal;
...
begin
   ...
   begin
     ...
     dwResult := WaitForMultipleObjects (1, @H, False, (SecondWaitFor*1000) );
    ...
    begin
        ...
        begin
           ...
           begin
                  ...
                  if  (P.BSource[CSource] =   SymbolBegin)
        ...
        begin
         ...
         //  showmessage(ReEtap.ToString +'  ' +P.BSource[CSource].ToString  +' '+CSource.ToString+' '+ CountBSource.ToString);
               ...
               ReLast:= CSource;
               ReFerst:= CSource - CPatern - ReEnd   -1{SymbolBegin} -1{SymbolEnd} ;
               ReFerst:=ReFerst;
...
begin
    Callback:=0;
    Result:= PosBytes_IncCallback(BPatern,BSource,CountBPatern,CountBSource,Callback);
end;
function PosBytes_IncCallback (BPatern:Tbytes;BSource:Tbytes;CountBPatern,CountBSource:int64;var Callback:int64):int64;
...
begin
    ...
    if  not  IsUTF8String(Result) then           Result:=AnsiToUtf8(Result)
    else if  HasExtendCharacter(Result)  then    Result:=UTF8ToString(Result);
end;
...
function amStr(value:Tcolor):string;  begin   result:= ColorToString(value); end;

function amStr(value:Tdatetime):string; begin   result:= dateTimeToStr(value); end;
